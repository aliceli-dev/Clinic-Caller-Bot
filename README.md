...
This places real phone calls and plays a patient. I ran about 10 full conversations against a clinic voice agent, then sat through the recordings and wrote up what broke. Each call gets an mp3 of both sides and a transcript.
...
The first thing anyone judging this will do is listen. The usual pipeline (STT, then GPT, then TTS) is cheaper, but two voice systems on a phone already add delay, and another three hops means the patient pauses after every sentence. ConversationRelay would have done STT/TTS for me; that's fine for a production agent, but here it would show up as dead air on the recording. So I used Realtime: audio in, audio out, one hop. The prompt is a persona and a goal, not a script. Realtime's job is to talk.
